            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-lg">
                <div class="overflow-x-auto">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($machineTypes as $machineType)
                                <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $machineType->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $machineType->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm"><a href="{{ route('machineTypes.edit', $machineType->id) }}" class="text-green-600 hover:underline font-medium">Update</a></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <form action="{{ route('machineTypes.destroy', $machineType->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <div class="flex justify-center mb-4">
                        <a href="{{ route('machineTypes.create') }}" 
